...
nub_size_t MachVMMemory::PageSize(task_t task) {
  if (m_page_size == kInvalidPageSize) {
    ...
    if (m_err.Fail()) m_page_size = 0;
  }
  return m_page_size;
}

nub_size_t MachVMMemory::MaxBytesLeftInPage(task_t task, nub_addr_t addr, nub_size_t count) {
  ...
  if (page_size > 0) {
    ...
    if (count > bytes_left_in_page) count = bytes_left_in_page;
  }
  return count;
}

#define MAX_STACK_ALLOC_DISPOSITIONS (16 * 1024 / sizeof(int)) // 16K of allocations

std::vector<nub_addr_t> get_dirty_pages(task_t task, mach_vm_address_t addr, mach_vm_size_t size) {
  ...
  mach_vm_size_t dispositions_size = std::min(pages_to_query, (int)MAX_STACK_ALLOC_DISPOSITIONS);
  int dispositions[dispositions_size];

  mach_vm_size_t chunk_count = ((pages_to_query + MAX_STACK_ALLOC_DISPOSITIONS - 1) / MAX_STACK_ALLOC_DISPOSITIONS);

  for (mach_vm_size_t cur_disposition_chunk = 0; cur_disposition_chunk < chunk_count; cur_disposition_chunk++) {
    mach_vm_size_t dispositions_already_queried = cur_disposition_chunk * MAX_STACK_ALLOC_DISPOSITIONS;

    mach_vm_size_t chunk_pages_to_query = std::min(pages_to_query - dispositions_already_queried, dispositions_size);
    mach_vm_address_t chunk_page_aligned_start_addr = addr + (dispositions_already_queried * vm_page_size);

    kern_return_t kr =
        mach_vm_page_range_query(task, chunk_page_aligned_start_addr, chunk_pages_to_query * vm_page_size,
                                 (mach_vm_address_t)dispositions, &chunk_pages_to_query);
    if (kr != KERN_SUCCESS) return dirty_pages;
    for (mach_vm_size_t i = 0; i < chunk_pages_to_query; i++) {
      uint64_t dirty_addr = chunk_page_aligned_start_addr + (i * vm_page_size);
      if (dispositions[i] & VM_PAGE_QUERY_PAGE_DIRTY) dirty_pages.push_back(dirty_addr);
    }
  }
  return dirty_pages;
}

nub_bool_t MachVMMemory::GetMemoryRegionInfo(task_t task, nub_addr_t address, DNBRegionInfo *region_info) {
  ...
  if (vmRegion.GetRegionForAddress(address)) {
    ...
    region_info->dirty_pages = get_dirty_pages(task, vmRegion.StartAddress(), vmRegion.GetByteSize());
    ...
    if (vmRegion.GetError().Success()) {
      ...
      if (address < start_addr) region_info->size = start_addr - address;
    }
    ...
    if (region_info->size == 0) region_info->size = INVALID_NUB_ADDRESS - region_info->addr;
...
nub_bool_t MachVMMemory::GetMemoryProfile(DNBProfileDataScanType scanType, task_t task, struct task_basic_info ti,
                                          ...
                                          uint64_t &memory_cap) {
  if (scanType & eProfileHostMemory) physical_memory = GetPhysicalMemory();
  ...
  if (scanType & eProfileMemoryCap) {
    ...
    if (memorystatus_control(MEMORYSTATUS_CMD_GET_MEMLIMIT_PROPERTIES, pid, 0, &memlimit_properties,
...
nub_size_t MachVMMemory::Read(task_t task, nub_addr_t address, void *data, nub_size_t data_count) {
  if (data == NULL || data_count == 0) return 0;
...
nub_size_t MachVMMemory::Write(task_t task, nub_addr_t address, const void *data, nub_size_t data_count) {
  MachVMRegion vmRegion(task);
  ...
  while (total_bytes_written < data_count) {
    if (vmRegion.GetRegionForAddress(curr_addr)) {
      ...
      if (region_bytes_left == 0) { break; }
      if (curr_data_count > region_bytes_left) curr_data_count = region_bytes_left;
...
nub_size_t MachVMMemory::WriteRegion(task_t task, const nub_addr_t address, const void *data,
  ...
  while (total_bytes_written < data_count) {
    mach_msg_type_number_t curr_data_count =
        static_cast<mach_msg_type_number_t>(MaxBytesLeftInPage(task, curr_addr, data_count - total_bytes_written));
