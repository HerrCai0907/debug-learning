...
public:
  ...
  struct Data {
    ...
    Data() : task_port(TASK_NULL), thread_port(THREAD_NULL), exc_type(0), exc_data() {}
    ...
    bool IsValid() const { return task_port != TASK_NULL && thread_port != THREAD_NULL && exc_type != 0; }
    ...
    void AppendExceptionData(mach_exception_data_t Data, mach_msg_type_number_t Count) {
  ...
  struct Message {
    ...
    kern_return_t Receive(mach_port_t receive_port, mach_msg_option_t options, mach_msg_timeout_t timeout,
